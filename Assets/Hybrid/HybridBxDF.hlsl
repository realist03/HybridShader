#ifndef HYBRID_BXDF_INCLUDED
#define HYBRID_BXDF_INCLUDED
...
float3 Diffuse_Burley( NPRSurfaceData surfaceData, float NoV, float NoL, float VoH ,float LoH)
{
	...
	float FdV = 1 + (FD90 - 1) * Pow5( 1 - NoV ).x;
	float FdL = 1 + (FD90 - 1) * Pow5( 1 - NoL ).x;
	float3 FD = saturate(FdV * FdL);
    //float FL = SchlickFresnel(NoL), FV = SchlickFresnel(NoV);
    float Fss90 = LoH*LoH*surfaceData.roughness;
	float FsV = 1 + (Fss90 - 1) * Pow5( 1 - NoV ).x;
	float FsL = 1 + (Fss90 - 1) * Pow5( 1 - NoL ).x;
...
{
	float Fc = Pow5( 1 - VoH ).x;					// 1 sub, 3 mul
				  ...
				  float XoL, float YoL, float XoV, float YoV, float XoH, float YoH)
{
	...
	//Gs = smithG_GGX(NoV,surfaceData.roughness);
...
half3 HybridPBRLit(NPRInputData inputData, NPRSurfaceData surfaceData, Light light)
{
	BxDFContext Context;
...
half4 HybridPBR(NPRInputData inputData, NPRSurfaceData surfaceData)
{
//DirectLight
	Light light = GetMainLight(inputData.shadowCoord);
	half4 color = half4(HybridPBRLit(inputData,surfaceData,light),1);
    ...
    for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
    {
        ...
        color +=  half4(HybridPBRLit(inputData,surfaceData,light),1);
    }
#endif
